...
exports.createCheckout = asyncHandler(async (req, res) => {
  ...
  const selected = packages[req.body.package];
  if (!selected) {
  ...
  const payment = await Payment.create({ owner: req.user._id, invitation: req.body.invitation, package: req.body.package, amount: selected.amount });
  const session = await stripe.checkout.sessions.create({
    ...
    metadata: { paymentId: String(payment._id), userId: String(req.user._id), package: req.body.package }
  });
...
exports.webhook = asyncHandler(async (req, res) => {
  ...
  if (event.type === 'checkout.session.completed') {
    ...
    const payment = await Payment.findByIdAndUpdate(session.metadata.paymentId, { status: 'paid' }, { new: true });
    if (payment) await User.findByIdAndUpdate(payment.owner, { plan: payment.package });
  }
